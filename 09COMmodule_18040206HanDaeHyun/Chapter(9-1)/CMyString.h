...
public :
	...
	friend bool CMyString::operator==(CMyString & c1, CMyString & c2);
	friend istream & CMyString::operator>>(istream & is, CMyString & c1);
	operator char*() const;
	void CMyString::SetStringSize();
};
...
{
	delete [] m_pcString;
	m_pcString = new char [ c1.getSize() + 1 ];
	strcpy( m_pcString, c1 );
